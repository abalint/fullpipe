...
pulls, taps, marks watched. The ledger row keeps `series` /
`ep_no` and uses the series title as its `channel`. **Rating is per series,
not per episode** (2026-09-20): the survey block never appears on a series
row or in the player for a series episode; instead the series header (and
the player's close-out) carries a 👎 / 👍 pair — tap the side you're on to
toggle single ⇄ double, tap the other side to switch — stored as one thumbs
verdict for the whole set (`POST /series/{slug}/rating`, ledger
`series_taste`). Nothing clears it from the phone; the verdict only moves.
...
phone** (at least one episode downloaded) and **Not on phone** (nothing
downloaded; starts folded). Inside a shelf each series keeps its own
collapsible header (title · n/N watched · m on phone · `▶ EPnn` / `⬇ EPnn`
for the next unwatched episode · the set's 👎 / 👍 thumbs) over its rows in
`ep_no` order with an `EPnn` chip; sort/filter still apply per row. Fold state is remembered per section,
shelf and series (`fp.series.*` in localStorage). When an
